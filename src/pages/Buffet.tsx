...
import { EditProductDialog } from '@/components/buffet/EditProductDialog';
...
const Buffet = () => {
  const { products, loading, createSale, addProduct, updateProduct, deleteProduct } = useProducts();
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4">
                  {filteredProducts.map((product, index) => (
                    <div
                      key={product.id}
                      className={cn(
                        'glass glass-hover rounded-xl p-4 transition-all duration-300 animate-fade-in relative group',
                        product.stock > 0 
                    ...
                    >
                      {/* Edit Button - Only visible on hover for staff */}
                      {isStaffOrAdmin && (
